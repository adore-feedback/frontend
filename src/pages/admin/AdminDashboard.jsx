...
const PersonalizedLinksPanel = ({
  ...
  return (
    <div style={{ marginTop: 4 }}>
      ...
      >
        Cryptographically signed links — only the recipient can open &amp;
        submit. Expires in <strong>7 days</strong>.
      </p>
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                >
                  …?token=•••
                  {t.name
                    ? `&for=${encodeURIComponent(t.name)}`
                    : t.company
                      ? `&for=${encodeURIComponent(t.company)}`
                      : ""}
                </code>
                <code
                  style={{
  ...
  <div style={S.overlay} onClick={onCancel} role="presentation">
    ...
    >
      ...
      <div style={S.modalActions}>
        <button style={S.btnCancel} onClick={onCancel} disabled={isDeleting}>
          Cancel
        </button>
  ...
  <div style={S.overlay} onClick={onCancel} role="presentation">
    ...
    >
      ...
      <p style={S.modalDesc}>
        ...
        <strong style={{ color: "#0f172a" }}>"{form?.title}"</strong> will be
        erased from the database entirely — no recovery possible.
      </p>
      {error && <p style={S.modalError}>{error}</p>}
      <div style={S.modalActions}>
        <button style={S.btnCancel} onClick={onCancel} disabled={isDeleting}>
...
const SettingsModal = ({ form, onSave, onCancel, isSaving, saveError }) => {
  ...
  const [opensAt, setOpensAt] = useState(
    toLocalDatetimeValue(form.availability?.opensAt),
  );
  const [closesAt, setClosesAt] = useState(
    toLocalDatetimeValue(form.availability?.closesAt),
  );
  const [slugVal, setSlugVal] = useState(slug);
  const [allowedList, setAllowedList] = useState(
    (form.allowedRespondents || []).join("\n"),
  );
  const [duplicateFields, setDuplicateFields] = useState(
    (form.duplicateCheckFields || ["email"]).join(", "),
  );
  ...
  const updateP = (email, field, value) => {
    ...
    setPersonalizations((prev) => [
      ...prev.filter((p) => p.identifier !== email),
      updated,
    ]);
  };
  ...
  const handleSave = () => {
    const payload = {
      ...
      visibility,
      slug:
        slugVal.trim() && slugVal.trim() !== slug ? slugVal.trim() : undefined,
      availability: {
      ...
      personalizations,
      title: internalTitle.trim() || undefined,
      duplicateCheckFields: duplicateFields
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean),
    };
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <h3 style={{ ...S.modalTitle, margin: 0, fontSize: 15 }}>
                Form Settings
              </h3>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              {["draft", "live", "closed"].map((s) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    fontWeight: 700,
                    border: `1.5px solid ${status === s ? STATUS_META[s].color : "#e2e8f0"}`,
                    background: status === s ? STATUS_META[s].bg : "#f8fafc",
                    color: status === s ? STATUS_META[s].text : "#64748b",
                    cursor: "pointer",
          ...
          {/* Internal Title rename */}
          <div>
            {fieldLabel(
              "Internal Form Title",
              "Visible only to admins — not shown to respondents",
            )}
            <input
              type="text"
              ...
              placeholder="Form title for internal reference"
            />
          </div>
          ...
          <div>
            {fieldLabel(
              "Form URL",
              "Only change this if you want a custom URL — changing it breaks existing shared links",
            )}
            ...
            {slugVal && slugVal !== slug && (
              ...
              >
                ...
                >
                  ⚠️ Changing slug will break existing shared links!
                </code>
                <button
                  type="button"
                  style={S.copyBtn}
                  onClick={() => setSlugVal(slug)}
                >
                  Reset
                </button>
              </div>
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    fontWeight: 700,
                    border: `1.5px solid ${visibility === v ? "#3b82f6" : "#e2e8f0"}`,
                    background: visibility === v ? "#eff6ff" : "#f8fafc",
                    color: visibility === v ? "#1d4ed8" : "#64748b",
                    cursor: "pointer",
                    transition: "all 0.15s",
                  }}
                >
          ...
          {visibility === "restricted" && (
            <div>
              ...
              <p style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
                {respondentEmails.length} email
                {respondentEmails.length !== 1 ? "s" : ""} listed
              </p>
            </div>
          )}
          ...
          {visibility === "restricted" && respondentEmails.length > 0 && (
            <div>
              {fieldLabel(
                "Personalization",
                "Set greeting names and pre-fill company details per respondent.",
              )}
              <div
                style={{
                  display: "flex",
          ...
          <div>
            {fieldLabel(
              ...
              "Comma-separated: email, phone, uniqueId",
            )}
            <input
              type="text"
              style={S.settingsInput}
        ...
        {saveError && (
          <p style={{ ...S.modalError, marginTop: 16 }}>{saveError}</p>
        )}

        <div style={{ ...S.modalActions, marginTop: 22 }}>
          <button style={S.btnCancel} onClick={onCancel} disabled={isSaving}>
            Cancel
          </button>
          <button
            style={{
...
const MobileAnalyticsDrawer = ({
  ...
  const avgRating =
    analytics.averageRating || selectedForm?.averageRating || null;
        ...
        <div style={S.sideCard}>
              ...
              >
                ...
                <span
                  style={{
                    ...S.statusBadge,
                    background:
                      STATUS_META[selectedForm.status]?.bg || "#f8fafc",
                    color: STATUS_META[selectedForm.status]?.text || "#64748b",
                  }}
                >
                  <span
                    style={{
                      ...
                      borderRadius: "50%",
                      background:
                        STATUS_META[selectedForm.status]?.color || "#94a3b8",
                      display: "inline-block",
                    }}
                  />
          ...
          >
            <div style={S.miniStat}>
              <span style={S.miniVal}>
                {(analytics.totalResponses || 0).toLocaleString()}
              </span>
              <span style={S.miniLabel}>Responses</span>
            </div>
            <div style={S.miniStat}>
              <span style={S.miniVal}>{avgRating || "—"}</span>
...
const AdminDashboard = () => {
  ...
  const requestPermanentDelete = useCallback((e, form) => {
    e.stopPropagation();
    setDeleteError("");
    setPendingPermanentDelete(form);
  }, []);
  ...
  const confirmPermanentDelete = useCallback(async () => {
    ...
    try {
      ...
      setDeleteError(
        err.message || "Could not permanently delete. Please try again.",
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  const saveSettings = useCallback(async (id, payload) => {
    ...
    try {
      ...
      setForms((prev) =>
        prev.map((f) => (getFormId(f) === id ? { ...f, ...updated.form } : f)),
      );
      // Also update selectedResult's form reference if this was the selected form
      if (selectedFormId === id) {
        setSelectedResult((prev) => ({
          ...prev,
          form: { ...(prev.form || {}), ...updated.form },
        }));
      }
  ...
  const selectedForm = useMemo(
    () => forms.find((f) => getFormId(f) === selectedFormId) || null,
        ...
        <div style={S.topBar}>
          <div>
            ...
            <p style={S.pageSubtitle}>
              {forms.length} form{forms.length !== 1 ? "s" : ""} · {liveCount}{" "}
                  ...
                  {filteredForms.length === 0 ? (
                    <tr>
                      <td colSpan="4" style={S.emptyRow}>
                        {searchQuery ? (
                          <>
                            No forms match "<strong>{searchQuery}</strong>" —{" "}
                            <button style={S.clearLink} onClick={clearSearch}>
                              clear search
                            </button>
                          </>
                        ) : filterStatus === "deleted" ? (
                          <span style={{ color: "#94a3b8" }}>
                            No deleted forms — your delete tab is clean 🎉
                          </span>
                        ) : (
                          "No forms found."
                        )}
                        ...
                        >
                          ...
                          <td style={{ ...S.td, textAlign: "right" }}>
                            ...
                            >
                              ...
                              {!isDeleted && (
                                <button
                                  onClick={(e) => openSettings(e, form)}
                                  style={S.actionSettings}
                        ...
                        <span
                          style={{
                            ...
                            background:
                              STATUS_META[selectedForm.status]?.bg || "#f8fafc",
                            color:
                              STATUS_META[selectedForm.status]?.text ||
                              "#64748b",
                          }}
                        >
                          <span
                            style={{
                              ...
                              borderRadius: "50%",
                              background:
                                STATUS_META[selectedForm.status]?.color ||
                                "#94a3b8",
                              display: "inline-block",
                            }}
                          />
