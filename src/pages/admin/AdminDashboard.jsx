      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            {fieldLabel(
              "Form Slug (URL)",
              "Changes the shareable URL: /form/{slug}",
            )}
...
const AdminDashboard = () => {
  ...
  /**
   * KEY LOGIC:
   * - If the form has NO questions, is NOT live, and has 0 responses → permanently delete immediately (no soft delete step).
   * - Otherwise → soft delete (status = "deleted") so responses are preserved and visible in the Deleted tab.
   * - Already-deleted forms in the Deleted tab get a separate "🗑️ Erase" button that calls confirmPermanentDelete.
   */
const confirmDelete = useCallback(async () => {
  ...
  try {
    if (!hasResponses) {
      // ✅ No responses (questions ho ya na ho) → permanently delete
      await deleteForm(id);          // Step 1: soft delete (backend requirement)
      await permanentDeleteForm(id); // Step 2: hard delete
      setForms((prev) => prev.filter((f) => getFormId(f) !== id));
    } else {
      // ✅ Has responses → soft delete only → moves to Deleted tab
      await deleteForm(id);
      setForms((prev) =>
        prev.map((f) =>
  ...
  // "all" tab shows active forms only (live/draft/closed); "deleted" tab shows only deleted
                        ...
                        >
                          ...
                          <td style={{ ...S.td, textAlign: "right" }}>
                            ...
                            >
                              ...
                              {/* ── Permanent Erase — only for already-deleted forms ── */}
                              {isDeleted && (
                                <button
                                  onClick={(e) =>
                                  ...
                                  title="Permanently erase this form and all its data"
                                >
                                  🗑️ Erase
                                </button>
                              )}
