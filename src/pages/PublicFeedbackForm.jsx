...
const getSavedRespondentEmail = () => {
  try {
    return localStorage.getItem(RESPONDENT_EMAIL_KEY) || "";
  } catch {
    return "";
  }
};
...
const getSavedRespondentName = () => {
  try {
    return localStorage.getItem(RESPONDENT_NAME_KEY) || "";
  } catch {
    return "";
  }
};
...
const HybridMultiSelect = ({ question, value, onChange }) => {
  ...
  const selectedCount =
    (val.selected?.length || 0) + (val.customText?.trim() ? 1 : 0);
      ...
      <div style={S.centeredPage}>
        ...
        <div style={S.glassCard}>
          ...
          <p style={S.gateSubtitle}>
            ...
            requires verification. Continue with your saved account?
          </p>
          ...
          {error && (
            <div style={S.errorBox}>
              <span>⚠️</span> {error} This account may not be on the access
              list.
            </div>
          )}
          <button
          ...
          >
            {isVerifying ? (
              <>
                <span style={S.spinnerEl} /> Verifying…
              </>
            ) : (
    ...
    <div style={S.centeredPage}>
      ...
      <div style={S.glassCard}>
        ...
        <p style={S.gateSubtitle}>
          ...
          is restricted to invited respondents only.
        </p>
        <form
        ...
        >
          ...
          <button
            ...
            style={{
              ...S.primaryBtn,
              marginTop: 4,
              opacity: isVerifying || !email.trim() ? 0.55 : 1,
            }}
            className="primary-btn"
          >
            {isVerifying ? (
    ...
    <div style={S.glassCard}>
      <div
        style={{
          ...
          justifyContent: "center",
          margin: "0 auto 24px",
          boxShadow: `0 8px 24px ${iconBg}60`,
        }}
      >
        {icon}
      </div>
...
const AccessDeniedScreen = ({ email, onRetry }) => (
  ...
  >
    ...
    >
      Contact the form administrator if you believe this is an error.
    </p>
    <button
      type="button"
      style={{ ...S.primaryBtn, background: "#374151" }}
      className="primary-btn"
      onClick={onRetry}
    >
...
const InvalidTokenScreen = ({ message }) => (
  ...
  >
    ...
    <p style={S.gateSubtitle}>
      {message ||
        "This form requires a personalised invitation link. Please use the link sent to you directly."}
    </p>
  </StatusScreen>
);
...
const AlreadyResponded = ({ formTitle, respondentName, respondentEmail }) => (
  ...
  >
    ...
    <p style={S.gateSubtitle}>
      ...
      <strong style={{ color: "#6366f1" }}>{formTitle}</strong>.
    </p>
    {respondentEmail && (
      <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 8px" }}>
        Submitted as{" "}
        <strong style={{ color: "#374151" }}>{respondentEmail}</strong>
      </p>
    )}
    <p
      style={{
...
const SessionExpired = ({ onRetry }) => (
  ...
  >
    ...
    <p style={S.gateSubtitle}>
      Your session has timed out. Please reopen your personalised link to
      continue.
    </p>
    <button
      type="button"
...
const PublicFeedbackForm = () => {
  ...
  const loadForm = useCallback(
    async (accessEmail = null, accessName = null) => {
      ...
      try {
        ...
        if (isPersonalizedLink) {
          ...
          const prefillName =
            serverPrefill.name || accessName || gateName || "";
  ...
  const flattenAnswer = (q, raw) => {
    ...
    if (q.type === "single-choice") {
      const customText = answers[`${q.id}_custom`]?.trim();
      if (customText && !raw) return customText;
      if (customText && raw) return [raw, customText];
  ...
  if (gateState === "gate" || gateState === "verifying") {
    const gatePrefilledEmail =
      gateEmail || savedAccessForForm?.email || savedEmail || "";
    const gatePrefilledName =
      gateName || savedAccessForForm?.name || savedName || "";
    return (
      <EmailIdentityGate
    ...
    return (
      ...
      >
        ...
        {submittedEmail && (
          <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 8px" }}>
            Submitted as{" "}
            <strong style={{ color: "#374151" }}>{submittedEmail}</strong>
          </p>
        )}
        {!submittedEmail && (
          <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 8px" }}>
...
const S = {
  ...
  glassCard: {
    ...
    width: "100%",
    boxShadow:
      "0 4px 6px -1px rgba(0,0,0,0.05), 0 20px 60px -10px rgba(0,0,0,0.08)",
  ...
  heroBg: {
    ...
    inset: 0,
    backgroundImage:
      "radial-gradient(ellipse at 75% 40%, rgba(99,102,241,0.22) 0%, transparent 55%), radial-gradient(ellipse at 15% 85%, rgba(139,92,246,0.15) 0%, transparent 50%)",
    pointerEvents: "none",
  },
  heroOrb1: {
    position: "absolute",
  ...
  pollRowSelected: { borderColor: "#6366f1", background: "#f0f0fe" },
  pollLetter: {
    width: 28,
    height: 28,
    borderRadius: "50%",
